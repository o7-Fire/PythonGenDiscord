...
class TokenGenerator:
    def __init__(self, verbose, proxy, workerID="idk"):
        self.id = str(workerID)
        ...
        if self.VerboseOutput:
            print("[{}] Obtained __dcfduid cookie! ({})".format(self.id, dcfduid))
            print("[{}] Obtained __sdcfduid cookie! ({})".format(self.id, sdcfduid))
        ...
        if self.VerboseOutput:
            print("[{}] Obtained fingerprint! ({})".format(self.id, fingerprint))
        self.fingerprint = fingerprint

        user_agent = "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:90.0) Gecko/20100101 Firefox/90.0"
        build_num, build_hash, build_id = getClientData('stable')#wtf is this
        try:
